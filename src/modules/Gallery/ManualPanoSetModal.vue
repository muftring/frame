  ...
  <div class="editor-backdrop" @click.self="$emit('close')">
    <div class="editor-modal" @click.stop>
      ...
      <div class="editor-header">
        <h2 class="editor-title">New Panorama Set</h2>
      ...
      <div class="editor-body">
        ...
        <div class="field-group">
          ...
          <div v-else-if="!availableFiles.length" class="file-list-empty">
            No unassigned files in this session — every file already belongs to a panorama set.
          </div>
      ...
      <div class="editor-footer">
        ...
        <button class="btn-primary" @click="create" :disabled="creating || !name.trim() || selectedIds.length < 2">
          {{ creating ? 'Creating…' : 'Create Panorama Set' }}
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    async create() {
      ...
      this.creating = true
      try {
        await window.api.invoke('pano:confirmSet', this.activeSession.id, [...this.selectedIds], this.name.trim())
        this.$emit('created')
      } finally {
        this.creating = false
      }
